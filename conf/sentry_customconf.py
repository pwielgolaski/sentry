...
if SENTRY_USE_REMOTE_USER:
    ...
    def build_LAZY_CUSTOM_REMOTE_USER_BACKEND(header, *args, **kwargs):
        from django.contrib.auth.backends import RemoteUserBackend
        class CustomRemoteUserBackend(RemoteUserBackend):
            def configure_user(self, user):
                print "Configure User"
                user.email = "{0}@{1}".format(user.username, "test.com")
                user.is_managed = True
                user.is_superuser = True
                user.save()
                return user
        return CustomRemoteUserBackend(*args, **kwargs)
    LAZY_CUSTOM_REMOTE_USER_BACKEND = functools.partial(build_LAZY_CUSTOM_REMOTE_USER_BACKEND, AUTH_REMOTE_USER_HEADER)
    AUTHENTICATION_BACKENDS += ('sentry_config.LAZY_CUSTOM_REMOTE_USER_BACKEND',)
